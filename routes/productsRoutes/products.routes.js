...
router.get('/get-products', (req, res, next) => {
	Product.find({ activeProduct: true, inSale: true })
...
router.post('/create-product/:user_id', (req, res, next) => {
	...
	const { name, description, price, stateOfProduct, images, category, subcategory } =
		req.body
	...
	Product.create(newProd)
		.then((product) => {
			User.findByIdAndUpdate(
				user_id,
				{ $addToSet: { sellingProducts: product._id } },
				{ new: true }
			)
...
router.put('/delete/:product_id', (req, res, next) => {
	...
	Product.findByIdAndUpdate(product_id, { activeProduct: false }, { new: true })
